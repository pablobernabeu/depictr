...
delegates to it and redraws the result, leaving the computation where it is. The
prose follows British spelling, and `spelling::spell_check_package()` keeps the
word list tidy.

The bundled datasets are generated by `data-raw/generate_datasets.R`, and the
fitted objects in `inst/extdata/` by the cluster scripts in `dev/hpc/`. Both
directories are excluded from the build but are the only record of how those
files were produced, so keep them in step when the artefacts are regenerated.
...
templates in `.github/issue-templates/`, and comment on it on each subsequent
