...
#'   `"stack"` or `"dodge"`.
#' @param palette Colours for the groups; defaults to [modelviz_palette()].
...
#' @export
#' @examples
#' distribution_plot(lexical_decision, RT)
#' distribution_plot(lexical_decision, RT, group = condition, type = "density")
distribution_plot <- function(data, x, group = NULL,
  ...
  if (!is.null(group)) {
    pal <- palette %||% modelviz_palette(length(unique(data[[group]])))
    p <- p +
