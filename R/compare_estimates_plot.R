...
#'   `compare_estimates_plot(Frequentist = m1, Bayesian = m2)`).
...
#' @param labels Optional display labels (see [coef_plot()]).
...
#' @param palette Colours for the sources; defaults to [modelviz_palette()].
...
compare_estimates_plot <- function(...,
                                    names = NULL,
                                    conf_level = 0.95,
                                    intercept = FALSE,
  ...
  pal <- palette %||% modelviz_palette(length(src_names))
