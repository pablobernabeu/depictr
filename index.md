...
scheduled GitHub Actions keep it healthy between releases: a daily check
of the package and its full test suite against both the current and the
development versions of its dependencies, a weekly check of every URL in
the documentation and `R CMD check` on every push across Linux, macOS
and Windows.
