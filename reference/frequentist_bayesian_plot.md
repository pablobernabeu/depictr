...
blue (Bayesian) styling. This is a convenience wrapper around
...
- note_frequentist_no_prior:
  ...
  If `TRUE`, append "(no prior)" to the frequentist legend label –
  useful when the title names the Bayesian prior.
...
commonly for Bayesian fits, a tidy data frame of posterior summaries
(with columns such as `term`, `estimate`, `conf.low`/`conf.high`, or the
`Estimate`, `l-95% CI`, `u-95% CI` produced by `brms::fixef()`). The
function aligns terms by name, so small differences in naming between
the two sources only need to be reconciled in the term labels.
